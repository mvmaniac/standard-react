...
import type { NestedComment } from '@/shared/types';
...
import { useDeleteComment } from '@/queries/comments/use-delete-comment.ts';
...
export default function CommentItem(props: NestedComment) {
  ...
  const avatarUrl = props.author.avatar_url ?? defaultAvatar;
  ...
  return (
    <div className={cn('flex flex-col gap-8 pb-5', isRootComment ? 'border-b' : 'ml-6')}>
      <div className="flex items-start gap-4">
        <Link to={'#'}>
          <div className="flex h-full flex-col">
            <img src={avatarUrl} alt={avatarUrl} className="h-10 w-10 rounded-full object-cover" />
          </div>
        </Link>
        <div className="flex w-full flex-col gap-2">
          ...
          <div className="text-muted-foreground flex justify-between text-sm">
            <div className="flex items-center gap-2">
              <div onClick={toggleIsReply} className="cursor-pointer hover:underline">
                댓글
              </div>
            ...
            <div className="flex items-center gap-2">
              {isMine && (
                <>
                  <div onClick={toggleIsEditing} className="cursor-pointer hover:underline">
                    수정
                  </div>
                  <div className="bg-border h-3.25 w-0.5"></div>
